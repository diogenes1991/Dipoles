...
void Analysis::ReweightEvent(FourVector* ExtMom, double* ExtMass, int* ExtPID, int BornNext, double* weight){
    ...
    // std::vector<FourVector> final_bs_mom;
    // for(int i=2;i<BornNext;i++){
    //     if(std::abs(ExtPID[i])==5)final_bs_mom.push_back(ExtMom[i]);
    // }

    // for(FourVector p : final_bs_mom){
